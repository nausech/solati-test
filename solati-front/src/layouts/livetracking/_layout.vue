<template>
    <div>
        ...
        <div class="app-content content">
            ...
            <div  class="content-wrapper" style="margin-top: 120px !important;">
            <!--:class="{'content-menu-collapsed' : showSidebar}"-->    
...
export default {
    ...
    created(){
        ...
        const orders = this.ordersPending.concat(this.ordersAccepted)

        orderRef.on('child_added', (child) => {

            const newOrder = child.val()
            ...
            if (allOrders.length == 0){
                ...
                if (newOrder.status_id == 1){
                    
                    
                    
                    this.$store.dispatch('getOrder',newOrder.id).then((order) => {
                        //console.log(order)
                        this.newOrderComming = order

                        this.$store.commit('SET_TOGGLE_MODAL_NEW_ORDER',true)
                        const audio = new Audio(require('../../assets/sounds/iphone_notification.mp3'))
                        audio.play()
